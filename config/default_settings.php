<?php

// system-wide default setting_values rows
return [
    [
        'setting_item' => 'languages',
        'key' => 'show_language_switcher',
        'value' => '1',
        'label' => 'settings.fields.show_language_switcher',
        'type' => 'bool',
        'sort_order' => 1,
        'autoload' => 0,
    ],
    [
        'setting_item' => 'languages',
        'key' => 'enabled_languages',
        'value' => '["de","en"]',
        'label' => 'settings.fields.enabled_languages',
        'type' => 'json',
        'sort_order' => 2,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'languages',
        'key' => 'default_language',
        'value' => 'de',
        'label' => 'settings.fields.default_language',
        'type' => 'string',
        'sort_order' => 3,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'languages',
        'key' => 'fallback_language',
        'value' => 'en',
        'label' => 'settings.fields.fallback_language',
        'type' => 'string',
        'sort_order' => 4,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'locale',
        'key' => 'app_locale',
        'value' => 'en',
        'label' => 'settings.fields.app_locale',
        'type' => 'lang_switcher',
        'sort_order' => 1,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'locale',
        'key' => 'default_locale',
        'value' => 'de_DE',
        'label' => 'settings.fields.default_locale',
        'type' => 'string',
        'sort_order' => 2,
        'autoload' => 0,
    ],
    [
        'setting_item' => 'locale',
        'key' => 'date_format',
        'value' => 'l, d.m.Y',
        'label' => 'settings.fields.date_format',
        'type' => 'date',
        'sort_order' => 3,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'locale',
        'key' => 'datetime_format',
        'value' => 'l, d.m.Y H:i',
        'label' => 'settings.fields.datetime_format',
        'type' => 'datetime',
        'sort_order' => 4,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'locale',
        'key' => 'timezone',
        'value' => 'Europe/Berlin',
        'label' => 'settings.fields.timezone',
        'type' => 'timezone',
        'sort_order' => 5,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'locale',
        'key' => 'default_currency',
        'value' => 'EUR',
        'label' => 'settings.fields.default_currency',
        'type' => 'currency',
        'sort_order' => 6,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'locale',
        'key' => 'multi_currency_mode',
        'value' => '0',
        'label' => 'settings.fields.multi_currency_mode',
        'type' => 'bool',
        'sort_order' => 7,
        'autoload' => 0,
    ],
    [
        'setting_item' => 'locale',
        'key' => 'first_day_of_week',
        'value' => '1',
        'label' => 'settings.fields.first_day_of_week',
        'type' => 'int',
        'sort_order' => 6,
        'autoload' => 0,
    ],
    [
        'setting_item' => 'appearance',
        'key' => 'theme',
        'value' => 'auto',
        'label' => 'settings.fields.theme',
        'type' => 'theme_switcher',
        'sort_order' => 1,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'color-branding',
        'key' => 'primary_color',
        'value' => '#3498db',
        'label' => 'settings.fields.primary_color',
        'type' => 'color',
        'sort_order' => 2,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'color-branding',
        'key' => 'secondary_color',
        'value' => '#64748B',
        'label' => 'settings.fields.secondary_color',
        'type' => 'color',
        'sort_order' => 3,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'color-branding',
        'key' => 'success_color',
        'value' => '#10b981',
        'label' => 'settings.fields.success_color',
        'type' => 'color',
        'sort_order' => 5,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'color-branding',
        'key' => 'danger_color',
        'value' => '#fa8072',
        'label' => 'settings.fields.danger_color',
        'type' => 'color',
        'sort_order' => 4,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'color-branding',
        'key' => 'border_radius',
        'value' => '10',
        'label' => 'settings.fields.border_radius',
        'type' => 'int',
        'sort_order' => 5,
        'autoload' => 0,
    ],
    [
        'setting_item' => 'color-branding',
        'key' => 'table_striped_rows',
        'value' => '1',
        'label' => 'settings.fields.table_striped_rows',
        'type' => 'bool',
        'sort_order' => 6,
        'autoload' => 0,
    ],
    [
        'setting_item' => 'color-branding',
        'key' => 'use_individual_module_colors',
        'value' => '1',
        'label' => 'settings.fields.use_individual_module_colors',
        'type' => 'bool',
        'sort_order' => 7,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'display-defaults',
        'key' => 'related_panel_limit',
        'value' => 5,
        'label' => 'settings.fields.related_panel_limit',
        'type' => 'int',
        'sort_order' => 1,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'display-defaults',
        'key' => 'list_view_limit',
        'value' => 31,
        'label' => 'settings.fields.list_view_limit',
        'type' => 'int',
        'sort_order' => 1,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'display-defaults',
        'key' => 'linking_panel_limit',
        'value' => 25,
        'label' => 'settings.fields.linking_panel_limit',
        'type' => 'int',
        'sort_order' => 1,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'company-info',
        'key' => 'company_name',
        'label' => 'settings.fields.company_name',
        'type' => 'string',
        'sort_order' => 1,
        'value' => 'Cubrel GmbH',
        'autoload' => 1,
    ],
    [
        'setting_item' => 'company-info',
        'key' => 'company_address',
        'label' => 'settings.fields.company_address',
        'type' => 'string',
        'sort_order' => 2,
        'value' => 'Ahornstraße  25, 19008 Berlin',
        'autoload' => 1,
    ],
    [
        'setting_item' => 'company-info',
        'key' => 'company_phone',
        'label' => 'settings.fields.company_phone',
        'type' => 'string',
        'sort_order' => 3,
        'value' => '+491265654651',
        'autoload' => 1,
    ],
    [
        'setting_item' => 'company-info',
        'key' => 'company_email',
        'label' => 'settings.fields.company_email',
        'type' => 'string',
        'sort_order' => 4,
        'value' => 'info@cubrel.com',
        'autoload' => 1,
    ],
    [
        'setting_item' => 'company-info',
        'key' => 'company_website',
        'label' => 'settings.fields.company_website',
        'type' => 'string',
        'sort_order' => 5,
        'value' => 'https://cubrel.com',
        'autoload' => 1,
    ],
    [
        'setting_item' => 'company-info',
        'key' => 'company_logo_url',
        'label' => 'settings.fields.company_logo_url',
        'type' => 'image',
        'sort_order' => 6,
        'value' => 'https://i.ibb.co/Gfnzq5cw/cover.png',
        'autoload' => 1,
    ],
    [
        'setting_item' => 'system',
        'key' => 'onboarding_completed',
        'value' => '0',
        'label' => null,
        'type' => 'bool',
        'sort_order' => 1,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'data-retention',
        'key' => 'retention_notifications_days',
        'value' => 180,
        'label' => 'settings.fields.retention_notifications_days',
        'type' => 'int',
        'sort_order' => 1,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'data-retention',
        'key' => 'retention_audit_logs_days',
        'value' => 730,
        'label' => 'settings.fields.retention_audit_logs_days',
        'type' => 'int',
        'sort_order' => 2,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'data-retention',
        'key' => 'retention_userinvites_days',
        'value' => 365,
        'label' => 'settings.fields.retention_userinvites_days',
        'type' => 'int',
        'sort_order' => 3,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'data-retention',
        'key' => 'retention_failed_jobs_days',
        'value' => 30,
        'label' => 'settings.fields.retention_failed_jobs_days',
        'type' => 'int',
        'sort_order' => 4,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'data-retention',
        'key' => 'retention_setup_tokens_days',
        'value' => 90,
        'label' => 'settings.fields.retention_setup_tokens_days',
        'type' => 'int',
        'sort_order' => 5,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'data-retention',
        'key' => 'retention_imports_days',
        'value' => 90,
        'label' => 'settings.fields.retention_imports_days',
        'type' => 'int',
        'sort_order' => 6,
        'autoload' => 1,
    ],
    [
        'setting_item' => 'data-retention',
        'key' => 'retention_draft_modules_days',
        'value' => 7,
        'label' => 'settings.fields.retention_draft_modules_days',
        'type' => 'int',
        'sort_order' => 7,
        'autoload' => 1,
    ],
];
