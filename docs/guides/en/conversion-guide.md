...
1. Give it a **name**, and pick the **Source Module** (what you're converting from) and **Target Module** (what gets created). Once saved, these two modules can't be changed — delete and recreate the rule if you picked wrong.
...
- **Field Mappings** — for each field on the target record, choose where its value comes from: a field on the source record, a fixed static value, or a small expression (literal text combined with a source field and/or a helper like today's date). A target field can only be mapped once.
