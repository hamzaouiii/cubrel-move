...
const rootStyle = computed(() =>
    props.accentColor ? { "--module-color": props.accentColor } : {},
);
...
const moduleLabel = computed(() =>
    t("modules." + props.moduleSlug + ".single_label"),
);
...
const recordLayout = computed(() => {
    const found = allLayouts.value.find((l) => l.module === props.moduleSlug)
        ?.layouts?.record;
...
const getField = (f) =>
    props.fields.find((field) => field.name === f.name) ?? f;
...
const getIcon = (slug) => {
    ...
    return (
        allModules.value.find((m) => m.slug === slug)?.icon ||
        "fa-solid fa-user"
    );
};
...
const getLinkingLayout = (slug) => {
    if (!slug) return null;
    return (
        allLayouts.value.find((l) => l.module === slug)?.layouts?.linkingPanel
            ?.columns || null
    );
};
...
const hasError = computed(
    () => (field) =>
        validationErrors.value.some((item) => item.field === field.name),
);
...
const submit = async () => {
    ...
    if (typeErrors.length > 0) {
        ...
        if (typeErrors.length > 1) {
            ...
            showError(
                t(typeErrors[0].label) +
                    " " +
                    t("fields.validation.invalid_format"),
            );
        }
        return;
    }
    ...
    try {
        ...
        clearAllAlerts();
        showError(
            e.response?.data?.message || t("modules.actions.create_error"),
        );
    } finally {
        saving.value = false;
    }
            ...
            <div class="quick-create-modal__container">
                <div class="quick-create-modal__header">
                    <div class="quick-create-modal__header__title">
                        <i :class="icon"></i>
                        <span
                            >{{ $t("modules.linking.create_record") }} —
                            {{ moduleLabel }}</span
                        >
                    </div>
                ...
                <div class="quick-create-modal__body">
                    <div class="record-layout__sections">
                        ...
                        >
                            <div
                                v-if="s.name"
                                class="record-layout__sections__item__title"
                            >
                                {{ s.name }}
                            </div>
                            ...
                            <div class="record-layout__sections__item__layout">
                                ...
                                >
                                    ...
                                    <FieldRenderer
                                        ...
                                        mode="edit"
                                        :related_label="
                                            form[f.name + '__label'] ?? null
                                        "
                                        :module-color="accentColor"
                                        :icon="
                                            getIcon(getField(f).related_module)
                                        "
                                        :has-error="hasError(f)"
                                        @update:model-value="
                                            (val) => updateField(f.name, val)
                                        "
