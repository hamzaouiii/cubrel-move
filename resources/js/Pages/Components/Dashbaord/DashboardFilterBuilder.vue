...
const scopeAppliesToUser = computed(
  () =>
    !user.value.is_admin &&
    !(dashboardConfig.value.org_wide_types ?? []).includes(user.value.type),
);
...
function operatorsFor(fieldName) {
  ...
  const ops =
    filterOperators.value.by_type?.[field?.type] ??
    filterOperators.value.default ??
    [];
  return ops.map((op) => ({
    value: op,
...
function valueField(fieldDef) {
  return {
    type: valueFieldType(fieldDef),
    related_module: fieldDef?.related_module ?? null,
  };
}
...
function valueRelatedField(fieldDef) {
  ...
  if (
    dashboardConfig.value.dropdown_field_types?.includes(fieldDef.type) &&
    fieldDef.dropdown_list
  ) {
...
const activeOverlayField = computed(() => {
  ...
  return getFieldDef(props.modelValue[activeConditionIndex.value]?.field);
});

function openValueOverlay(i) {
  activeConditionIndex.value = i;
...
function onValueRecordSelect(record) {
  if (activeConditionIndex.value === null) return;
  patchCondition(activeConditionIndex.value, {
    value: record.id,
...
function addCondition() {
  emit("update:modelValue", [
    ...props.modelValue,
    { field: null, operator: null, value: null, valueLabel: null },
  ]);
}
...
function patchCondition(i, delta) {
  emit(
    "update:modelValue",
    props.modelValue.map((c, idx) => (idx === i ? { ...c, ...delta } : c)),
  );
}
...
function onOperatorChange(i, op) {
  patchCondition(i, {
    operator: op,
    value: BETWEEN_OPERATORS.includes(op) ? [null, null] : null,
    valueLabel: null,
  });
}
  ...
  <div class="dfb" v-if="module">
    <div class="dfb__scope" v-if="scopeAppliesToUser">
      ...
      <div class="dfb__scope-text">
        <span class="dfb__scope-label">{{
          $t("globals.dashboard.show_all_records")
        }}</span>
        <span class="dfb__scope-hint">{{
          $t("globals.dashboard.show_all_records_hint")
        }}</span>
      </div>
    </div>
    ...
    <p v-if="!modelValue.length" class="dfb__empty">
      {{ $t("globals.dashboard.filter_empty") }}
    </p>
    ...
    <div v-else class="dfb__conditions">
      <div v-for="(cond, i) in modelValue" :key="i" class="dfb__row">
        ...
        <div class="dfb__cell dfb__cell--field">
          <FieldRenderer
            ...
            mode="dashboard"
            @update:modelValue="
              patchCondition(i, { field: $event, operator: null, value: null, valueLabel: null })
            "
          />
        </div>
        ...
        <div class="dfb__cell dfb__cell--op" v-if="cond.field">
          <FieldRenderer
            ...
            :related_field="{
              dropdown_list: { values: operatorsFor(cond.field) },
            }"
            mode="dashboard"
        ...
        >
          <!-- Between: two inputs -->
          <div
            v-if="BETWEEN_OPERATORS.includes(cond.operator)"
            class="dfb__cell dfb__cell--val dfb__cell--between"
          >
            <FieldRenderer
              :field="valueField(getFieldDef(cond.field))"
          ...
          <div v-else class="dfb__cell dfb__cell--val">
            <FieldRenderer
              :field="valueField(getFieldDef(cond.field))"
              :related_field="valueRelatedField(getFieldDef(cond.field))"
              :related_label="cond.valueLabel ?? null"
              :model-value="cond.value"
              mode="dashboard"
              @update:modelValue="patchCondition(i, { value: $event })"
              @open-link-overlay="openValueOverlay(i)"
            />
          </div>
        </template>
      ...
      <div class="dfb__match" v-if="modelValue.length > 1">
        <span class="dfb__match-label">{{
          $t("globals.dashboard.filter_match")
        }}</span>
        <div class="dfb__match-btns">
          ...
          >
            {{ $t("globals.dashboard.filter_match_all") }}
          </button>
          <button
            type="button"
            class="dfb__match-btn"
          ...
          >
            {{ $t("globals.dashboard.filter_match_any") }}
          </button>
        </div>
      </div>
    </div>
