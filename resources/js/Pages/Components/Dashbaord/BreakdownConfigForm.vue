...
const groupableFields = computed(() =>
  fields.value.filter(f => !['longtext', 'address'].includes(f.type))
)
...
const canSubmit = computed(() => {
  if (!form.module || !form.groupBy) return false
...
const groupByOptions = computed(() => groupableFields.value.map(f => ({ value: f.name, label: f.label })))
...
const chartTypeOptions  = buildOptions(t, dashboardConfig.breakdown_chart_types)

watch(() => form.module, async (slug) => {
  fields.value = []
  if (!slug) { form.groupBy = ''; form.metric.field = ''; return }
  loadingFields.value = true
  try {
    const { data } = await axios.get(`/dashboard/module-fields/${slug}`)
    fields.value = data
    if (!form.groupBy      && cfg.groupBy)       form.groupBy      = cfg.groupBy
    ...
    <template v-if="form.module">
      ...
      <div class="tsf__field">
        ...
        <FieldRenderer
          v-else
          :field="{ type: 'select' }"
          :model-value="form.groupBy || null"
          :related_field="{ dropdown_list: { values: groupByOptions } }"
          mode="dashboard"
          @update:modelValue="form.groupBy = $event ?? ''"
        />
      </div>
      ...
      <div class="tsf__row tsf__row--2col">
        ...
        <div class="tsf__field">
          <label class="tsf__label">{{ $t('globals.dashboard.field_max_groups') }}</label>
          <FieldRenderer
            :field="{ type: 'integer', min: 2, max: 50 }"
      ...
      <!-- Palette -->
      <div class="tsf__field">
        <label class="tsf__label">{{ $t('globals.dashboard.field_palette') }}</label>
