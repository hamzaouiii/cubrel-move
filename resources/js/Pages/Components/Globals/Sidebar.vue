...
// The module matching the current page, pinned to the top of the list
const currentModule = computed(() =>
  modules.value.find((mod) => currentUrl.value.startsWith(mod.path)),
);

const sortByCategory = computed(() => {
  const val = appSettings.value.sidebar_sort_by_category;
...
const remainingModules = computed(() =>
  modules.value.filter(
    (mod) => !currentModule.value || mod.slug !== currentModule.value.slug,
  ),
);

// Group modules by category, only used when sortByCategory is on.
const groupedModules = computed(() => {
  const groups = {};
  remainingModules.value.forEach((mod) => {
    const category = mod.category_id || "uncategorized";

    if (!groups[category]) {
      groups[category] = {
        label: mod.category_label || t("globals.sidebar.uncategorized"),
        modules: [],
      };
    }
    groups[category].modules.push(mod);
  });
  return groups;
});
    ...
    <div class="sidebar__module-list">
      ...
      <template v-if="currentModule">
        ...
        <Link
          ...
          "
          @mouseenter="onModuleMouseEnter($event, currentModule)"
          @mouseleave="onModuleMouseLeave"
        >
          <div class="sidebar__module-list__item__label active">
            <i
              :class="[
      ...
      <template v-if="sortByCategory">
        <template v-for="(group, category) in groupedModules" :key="category">
          <hr class="sidebar__module-list__divider" />
