...
const emit = defineEmits(["sent", "skip"]);
...
const color = computed(() => appSettings.primary_color);
...
const sendInvites = async () => {
  ...
  try {
    await axios.post("/invites/bulk", {
      invites: emails.value,
    });

    status.value = "success";
    setTimeout(() => emit("sent", emails.value), 3000);
  } catch (error) {
    status.value = "error";
    errorMessage.value =
...
<template>
  ...
  >
    <div class="invite-card__header" v-if="status !== 'success'">
      <div class="invite-card__title-group">
        <h2 class="invite-card__title">
          {{ $t("globals.onboarding.invite_title") }}
        </h2>
        <p class="invite-card__subtitle">
          {{ $t("globals.onboarding.invite_subtitle") }}
        </p>
      </div>
    ...
    <div class="invite-content" v-if="status === 'idle' || status === 'error'">
    ...
    <div class="invite-content invite-content--centered" v-if="status === 'success'">
      ...
      <div v-if="emails.length === 1">
        ...
        <p class="invite-success-desc">
          {{ $t("modules.users.modal.invitation_sent_desc", { total: emails.length }) }}
        </p>
      </div>
      <div v-else>
        <h3 class="invite-success-title">
        ...
        <p class="invite-success-desc">
          {{
            $t("modules.users.modal.invitation_sent_desc_plural", { total: emails.length })
          }}
        </p>
      </div>
    ...
    <div class="invite-card__footer" v-if="status !== 'success'">
      <div class="invite-card__footer__content">
        <span class="invite-count-text" v-if="status === 'idle' && emails.length > 0">
          <span v-if="emails.length === 1">
            {{ $t("modules.users.modal.total_to_invite", { total: emails.length }) }}
          </span>
          <span v-else>
            {{
              $t("modules.users.modal.total_to_invite_plural", { total: emails.length })
            }}
          </span>
        </span>
        <span v-else></span>
        ...
        <div class="invite-actions">
          ...
          >
            {{ $t("globals.onboarding.invite_skip") }}
          </button>
          <button
            @click="sendInvites"
            :disabled="status === 'sending' || emails.length === 0"
            class="invite-card__button"
            :class="{ 'invite-card__button--disabled': emails.length === 0 }"
          >
            {{
              status === "sending"
                ? $t("modules.users.modal.progress_text")
