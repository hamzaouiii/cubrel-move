...
const form = useForm({
  ...
  is_status: props.dropdown?.is_status ?? false,
});

// This list's own persisted flag decides which editor renders — a status
// list opened directly from the Dropdown Manager (not through a field)
// still needs to show the color/icon editor.
const isStatusMode = computed(() => form.is_status);

// Rows already saved on the server when the page loaded — their generated
// `value` (system key) must never be silently rewritten by later label
// edits, since other records may already reference it.
      ...
      <div class="settings__dropdown__edit">
        ...
        <ul>
          <li
            ...
            :class="{
              'status-drop-zone--active': dragIndex !== null && dragOverZone === 0,
            }"
            @dragover="onZoneDragOver(0, $event)"
            @drop="onZoneDrop(0)"
          ></li>
          <template v-for="(l, index) in form.values" :key="rowKey(l)">
          <li
            class="settings__dropdown__edit__value"
            :class="{
              'settings__dropdown__edit__value--status': isStatusMode,
              'settings__dropdown__edit__value--dragging': dragIndex === index,
            }"
          ...
          >
            <div class="settings__dropdown__edit__value__row">
              ...
              <div v-if="!isStatusMode" class="settings__dropdown__edit__value__actions">
                <span
                  class="settings__dropdown__edit__value__actions__delete"
                  @click="deleteItem(index)"
                >
