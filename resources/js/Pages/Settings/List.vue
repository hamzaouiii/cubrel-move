...
const settingsColumns = computed(() => {
    const columns = [[], []];
    Object.entries(filteredSettings.value).forEach(([key, group], index) => {
        columns[index % 2].push([key, group]);
    });
    return columns;
});
</script>
...
<template>
    ...
    >
        ...
        <div class="settings__list">
            <div
                v-for="(column, colIndex) in settingsColumns"
                :key="colIndex"
                class="settings__list__column"
            >
                <div
                    v-for="[key, s] in column"
                    :key="key"
                    class="settings__list__section"
                >
