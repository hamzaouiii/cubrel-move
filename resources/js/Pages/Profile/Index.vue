...
const getFieldByName = (f) => {
  const pp = props.fields.find((field) => field.name === f);
  return pp;
};
...
<template>
  <Head>
    <title>{{ record.first_name }} {{ record.last_name }} - Cubrel</title>
  </Head>
          ...
          <div class="record-layout__sections__item__layout">
            ...
            >
              ...
              <FieldRenderer
                ...
                v-model="form[f.name]"
                :mode="getMode(f)"
                :read-only="getField(f)?.readonly || f.readonly"
                :module-color="module_color"
